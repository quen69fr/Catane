...
PORT_3_CONTRE_1 = -2
LISTE_ID_ARETES_PORT_ET_ANGLE_PORT = [(10, 120), (11, 60), (28, 0), (59, 0), (70, 300), (68, 240), (51, 240), (34, 180), (20, 120)]
LISTE_RESSOURCES_PORTS = [RESSOURCE_PIERRE, PORT_3_CONTRE_1, RESSOURCE_MOUTON, PORT_3_CONTRE_1, PORT_3_CONTRE_1, RESSOURCE_ARGILE, RESSOURCE_BOIS, PORT_3_CONTRE_1, RESSOURCE_FOIN]
...
# ------------------------
class CaseACocher():
    def __init__(self, x, y, parametre, texte, taille=30, policeTexte=None, tailleTexte=30, couleurTexte=GRIS_FONCE, etat_initial=False, largeurTrai=3, couleurCase=GRIS_FONCE, couleurSelect=GRIS_MOYEN):
        self.x = x
        self.y = y
...
class Bouton():
    ...
    def affiche(self):
        if self.image == None:
            ...
            if self.selectionner or self.image == IMAGE_BOUTON_MESSAGES:
                pygame.draw.rect(SCREEN, self.couleurCountour, (self.x, self.y, self.largeur, self.hauteur), self.largeurContour)
...
class Ecran_reglages():
    def __init__(self):
        ...
        self.listeBoutonsJoueurs.append(Bouton(self.x + self.marge_x, self.y + self.marge_y, 'bleu', image=IMAGE_GRAND_JOUEUR, couleurRect=BLEU, largeurContour=5))
        self.listeBoutonsJoueurs.append(Bouton(self.x + self.marge_x * 2 + LARGEUR_IMAGE_GRAND_JOUEUR, self.y + self.marge_y, 'rouge', image=IMAGE_GRAND_JOUEUR, couleurRect=ROUGE, largeurContour=5))
        self.listeBoutonsJoueurs.append(Bouton(self.x + self.marge_x, self.y + self.marge_y * 2 + HAUTEUR_IMAGE_GRAND_JOUEUR, 'orange', image=IMAGE_GRAND_JOUEUR, couleurRect=ORANGE, largeurContour=5))
        self.listeBoutonsJoueurs.append(Bouton(self.x + self.marge_x * 2 + LARGEUR_IMAGE_GRAND_JOUEUR, self.y + self.marge_y * 2 + HAUTEUR_IMAGE_GRAND_JOUEUR, 'blanc', image=IMAGE_GRAND_JOUEUR, couleurRect=BLANC, largeurContour=5))
        m = 20
        h = 80
        ...
        l = 450
        self.listeBoutonsReglagePlateau.append(Bouton(X_CENTRE_AFFICHAGE_GAUCHE, int(self.y + m2 / 4 + h2 / 2), 'ressources', largeur=l, hauteur=h2, texte='Replacer les ressources', tailleTexte=55, couleurTexte=GRIS_FONCE, couleurRect=GRIS_CLAIR, couleurCountour=GRIS_CLAIR, centrer=True))
        self.listeBoutonsReglagePlateau.append(Bouton(X_CENTRE_AFFICHAGE_GAUCHE, int(self.y + 7 * m2 / 4 + h2 * 3 / 2), 'numeros', largeur=l, hauteur=h2, texte='Replacer les numéros', tailleTexte=55, couleurTexte=GRIS_FONCE, couleurRect=GRIS_CLAIR, couleurCountour=GRIS_CLAIR, centrer=True))
        self.listeBoutonsReglagePlateau.append(Bouton(X_CENTRE_AFFICHAGE_GAUCHE, int(self.y + 13 * m2 / 4 + h2 * 5 / 2), 'ports', largeur=l, hauteur=h2, texte='Replacer les ports', tailleTexte=55, couleurTexte=GRIS_FONCE, couleurRect=GRIS_CLAIR, couleurCountour=GRIS_CLAIR, centrer=True))

        self.listeCasesACaucherReglagePlateau = []
        m3 = 5
        self.listeCasesACaucherReglagePlateau.append(CaseACocher(X_CENTRE_AFFICHAGE_GAUCHE - l / 2 + m3, int(self.y + m2 / 4 + h2 + m3), 'ressources', 'Replacer manuellement les ressources'))
        self.listeCasesACaucherReglagePlateau.append(CaseACocher(X_CENTRE_AFFICHAGE_GAUCHE - l / 2 + m3, int(self.y + 7 * m2 / 4 + h2 * 2 + m3), 'numeros', 'Replacer manuellement les numéros'))
        self.listeCasesACaucherReglagePlateau.append(CaseACocher(X_CENTRE_AFFICHAGE_GAUCHE - l / 2 + m3, int(self.y + 13 * m2 / 4 + h2 * 3 + m3), 'ports', 'Replacer manuellement les ports'))
...
class Fenetre_banque():
    ...
    def affiche(self):
        ...
        for i in range(5):
            SCREEN.blit(listeImageCarteAAfficher[i], (self.x_affichage + self.marge + i * (LARGEUR_IMAGE_CARTE_DOS + self.marge), self.y_affichage + self.marge))
...
class Fenetre_echanges():
    def __init__(self, x, y, joueur, listeJoueurs):
        ...
        self.boutonValider = Bouton(self.x_affichage + marge, self.y + self.hauteur / 2 - marge - HAUTEUR_BOUTON_DANS_AFFICHAGE_PRINCIPAL, 'valider', largeur=int((self.largeur - 2 * marge - marge) / 2), hauteur=HAUTEUR_BOUTON_DANS_AFFICHAGE_PRINCIPAL, texte='VALIDER', couleurTexte=GRIS_FONCE, couleurRect=GRIS_CLAIR, couleurCountour=GRIS_CLAIR)
        self.boutonRefuser = Bouton(self.x_affichage + 2 * marge + int((self.largeur - 2 * marge - marge) / 2), self.y + self.hauteur / 2 - marge - HAUTEUR_BOUTON_DANS_AFFICHAGE_PRINCIPAL, 'refuser', largeur=int((self.largeur - 2 * marge - marge) / 2), hauteur=HAUTEUR_BOUTON_DANS_AFFICHAGE_PRINCIPAL, texte='REFUSER', couleurTexte=GRIS_FONCE, couleurRect=GRIS_CLAIR, couleurCountour=GRIS_CLAIR)
        ...
        for j in listeJoueurs:
            if j != joueur:
                self.listeBoutonJoueurs.append(Bouton(x_bouton, y_bouton, j.nom, largeur=150, hauteur=HAUTEUR_BOUTON_DANS_AFFICHAGE_PRINCIPAL, texte='les {}'.format(j.nom)))
    ...
    def affiche(self, joueur, joueurEchange):
        ...
        if self.principal == True:
            ...
            if self.etape >= 1:
                ...
                if self.etape >= 2:
                    joueur.afficheCartes(self.x + self.largeur - 110, self.y + 210, joueurEchange.cree_listeCartesSelectionnees(), 40)
                    self.boutonValider.affiche()
                    self.boutonRefuser.affiche()
        else:
            joueur.afficheCartes(self.x + 120, self.y + 210, joueur.cree_listeCartesSelectionnees(), 40)
            affiche_texte('avec les {} contre'.format(joueur.nom), self.x + self.largeur / 2, self.y, None, 30, NOIR, centrer=True)
            joueur.afficheCartes(self.x + self.largeur - 110, self.y + 210, joueurEchange.cree_listeCartesSelectionnees(), 40)
            self.boutonValider.affiche()
            self.boutonRefuser.affiche()
