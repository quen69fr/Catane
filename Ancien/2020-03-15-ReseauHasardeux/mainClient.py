...
if __name__=="__main__":
    ...
    if hote == '':
        ...
        m = 5
        msg = ''
        p = pygame.font.Font(None, h * 2 - 7 * m)
        ts = p.render(msg, True, NOIR)
        clic_up_down = 0
        b = Bouton(x, y + h + 15, '', l, h, texte='Valider', tailleTexte=50)
        reessayer = False
        while hote == '':
            ...
            for event in pygame.event.get():
                ...
                if clic_up_down == 1:
                    if event.type == pygame.MOUSEBUTTONUP:
                        clic_up_down = 0
                        if x < x_souris < x + l and y < y_souris < y + h:
                            actif = True
                        else:
                            if b.clic(x_souris, y_souris):
                                if msg != '':
                                    hote = msg
                            actif = False
                else:
            ...
            affiche_texte("Rentrer l'adresse IP du serveur", LARGEUR / 2 - MARGES, HAUTEUR_TITRE / 2 + 50, None, 52, GRIS_FONCE, centrer=True)
            rectangle(MARGES, 2 * MARGES + HAUTEUR_TITRE, LARGEUR - 2 * MARGES, HAUTEUR - 3 * MARGES - HAUTEUR_TITRE)
            c = GRIS_CLAIR
            if actif:
                c = GRIS_FONCE
            pygame.draw.rect(SCREEN, c, (x, y, l, h), 3)
            # self.bouton_envoyer.affiche()
            SCREEN.blit(ts, (int(x + l / 2 - ts.get_width() / 2), y + m))

            b.affiche()

            if reessayer:
                affiche_texte("La connection avec le serveur", LARGEUR / 2, y - 75, None, 34, ROUGE, centrer=True)
                affiche_texte("n'a pas pu être effectuée.", LARGEUR / 2, y - 46, None, 34, ROUGE, centrer=True)
                affiche_texte("Veuillez réessayer :", LARGEUR / 2, y - 17, None, 34, ROUGE, centrer=True)
    ...
    message = connexion_avec_serveur.recv(16384)
    monJoueur, etat_partie, listeJoueurs, listeCouleurJoueursPlacement, compteurJoueursPlacement, couleurJoueurTourJeu, sous_partie_jeu, nb_de1, nb_de2, voleur_etape, nature_clic, listePiocheCartesSpeciales, carte_speciale, etapeEtJoueurEchange, listeMessages = pickle.loads(message)
    if AFFICHAGE_MESSAGE:
    ...
    while True:
        try:
            ...
            for serveur in liste_serveur:
                ...
                for message_client_dumps in splitMessages(msg_complet):
        ...
        for joueur in listeJoueurs:
            ...
            if joueur == monJoueur:
                if clic:
                    if etapeEtJoueurEchange[0] == 0 or (etapeEtJoueurEchange[0] == 1 and maCouleur != couleurJoueurTourJeu) or (etapeEtJoueurEchange[0] == 2 and maCouleur != couleurJoueurTourJeu and etapeEtJoueurEchange[1] != monJoueur.nom):
        ...
        if etat_partie == 'placement':    # On fait comme si on pouvait placer tous les joueur
                    ...
                    if nature_clic == 'colonie':
                        ...
                        if sommet != None:
                            if plateau.placement_colonie_possible(sommet):
                                ...
                                envoieAuServeur('joueur', c, connexion_avec_serveur)
                                envoieAuServeur('joueur', monJoueur.nbPointsDeVictoire, connexion_avec_serveur, info=[monJoueur.nom, 'nbPointsDeVictoire'])
                                if plateau.ajoute_si_port_creation_colonie(sommet, monJoueur):
                                    envoieAuServeur('joueur', monJoueur.listeRessourcesPorts, connexion_avec_serveur, info=[monJoueur.nom, 'listeRessourcesPorts'])
                ...
                if nb_selec >= nb_a_selec:
                    ...
                    if nb_selec > nb_a_selec:
                        ...
                        if boutonJeterCartes.clic(x_souris, y_souris):
                            ...
                            envoieAuServeur('joueur', monJoueur.jeterCartes, connexion_avec_serveur, info=[monJoueur.nom, 'jeterCartes'])
            ...
            if couleurJoueurTourJeu == maCouleur:
                            ...
                            if clic:
                                if boutonUtiliserCartesSpeciale.clic(x_souris, y_souris):
                                    if carte_speciale_selectionner.ressource == POUVOIR_POINT_VICTOIRE:
                                        ...
                                        envoieAuServeur('joueur', monJoueur.nbPointsDeVictoire, connexion_avec_serveur, info=[monJoueur.nom, 'nbPointsDeVictoire'])
                                        envoieAuServeur('joueur', carte_speciale_selectionner, connexion_avec_serveur, info=monJoueur.nom)
                                    else:
                                        ...
                                        if carte_speciale == POUVOIR_MONOPOLE:
                                            ...
                                            envoieAuServeur('joueur', monJoueur.nbCartesChevaliersRetournees, connexion_avec_serveur, info=[monJoueur.nom, 'nbCartesChevaliersRetournees'])
                        ...
                        if clic:
                            ...
                            if ressource != None:
                                ...
                                for carte in listeCarte:
                                    ...
                                    envoieAuServeur('joueur', carte, connexion_avec_serveur, info=monJoueur.nom)
                                carte_speciale = None
                                c = monJoueur.carteSpecialesSelectionner()
                                monJoueur.listeCartesSpeciales.remove(c)
                                envoieAuServeur('joueur', c, connexion_avec_serveur, info=monJoueur.nom)
                                envoieAuServeur('carte_speciale', carte_speciale, connexion_avec_serveur)
                            ...
                            if bouton.selectionner:
                                ...
                                if clic:
                                    ...
                                    if ressource != None:
                                        ...
                                        id_carte += 4
                                        monJoueur.dictionnaireCartesRessources[ressource].append(c)
                                        envoieAuServeur('joueur', c, connexion_avec_serveur, monJoueur.nom)
                                        bouton.selectionner = False
                        ...
                        if nature_clic == 'colonie':
                            ...
                            if sommet != None:
                                if plateau.placement_ville_possible(sommet, monJoueur):
                                    ...
                                    envoieAuServeur('joueur', colonie, connexion_avec_serveur)
                ...
                if etapeEtJoueurEchange[1] == monJoueur.nom and etapeEtJoueurEchange[0] == 1:
                    for j in listeJoueurs:
                ...
                if not monJoueur.jeterCartes:
                    for j in listeJoueurs:
                        if j.couleur == couleurJoueurTourJeu:
                            ...
                            if couleur == BLANC:
                                couleur = NOIR
                            t = "Les {} sont en train de jouer.".format(j.nom)
                            if carte_speciale == POUVOIR_MONOPOLE:
                                t = 'MONOPOLE !'
                            elif carte_speciale == POUVOIR_CHEVALIER:
                                t = 'CHEVALIER !'
            ...
            if monJoueur.nbPointsDeVictoire >= 10:
                ...
                for joueur in listeJoueurs:
                    if joueur.nbPointsDeVictoire >= 10:
                        t = 'Les {} ont gagné !'.format(joueur.nom)
                        c = joueur.couleur
            if c == BLANC:
                c = NOIR
