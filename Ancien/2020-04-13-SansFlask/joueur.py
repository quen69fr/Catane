...
class Joueur():
    def __init__(self, couleur):
        ...
        self.listeRoutes = []
        # self.listeCartes = []
        self.dictionnaireCartesRessources = {RESSOURCE_BOIS: [], RESSOURCE_ARGILE: [], RESSOURCE_MOUTON: [], RESSOURCE_FOIN: [], RESSOURCE_PIERRE: []}
    ...
    # ----------------------------------------------------
    def deselection_cartes(self, connexion_avec_serveur):
        for cle in self.dictionnaireCartesRessources.keys():
            for carte in self.dictionnaireCartesRessources[cle]:
                if carte.selectionner:
    ...
    # ----------------------------------------------------
    def suprimeCartesSelectionnes(self, connexion_avec_serveur):
        for carte in self.cree_listeCartesSelectionnees():
            self.dictionnaireCartesRessources[carte.ressource].remove(carte)
            envoieAuServeur('joueur', carte, connexion_avec_serveur, info=[self.nom, '-'])

    # ----------------------------------------------------
    def rendCarteConstruction(self, prix, id_carte, connexion_avec_serveur):
        for ressource in prix:
            c = Carte(ressource, id_carte)
            id_carte += 4
            self.dictionnaireCartesRessources[ressource].append(c)
            envoieAuServeur('joueur', c, connexion_avec_serveur, info=[self.nom, '+'])
        return id_carte
    ...
    def afficheSecondaire(self, ySecondaire):
        ...
        m = 3
        pygame.draw.rect(SCREEN, self.couleur, (self.xSecondaire + m, ySecondaire + m, LARGEUR_IMAGE_JOUEUR, HAUTEUR_IMAGE_JOUEUR), 0)
        SCREEN.blit(IMAGE_JOUEUR, (self.xSecondaire + m, ySecondaire + m))
        c = self.couleur
        if c == BLANC:
            c = NOIR
        affiche_texte(self.pseudo.upper(), int(self.xSecondaire + m + LARGEUR_IMAGE_JOUEUR + (HAUTEUR_CARRE_JOUEUR_SECONDAIRE - m - LARGEUR_IMAGE_JOUEUR) / 2), int(ySecondaire + HAUTEUR_IMAGE_JOUEUR / 2 + m), None, 28, c, centrer=True)
        SCREEN.blit(IMAGE_CARTE_DOS, (self.xSecondaire + m - 2, ySecondaire + m + HAUTEUR_IMAGE_JOUEUR))
        affiche_texte(str(self.compte_nb_cartes()), int(self.xSecondaire + m  - 2 + LARGEUR_IMAGE_CARTE_DOS / 2), ySecondaire + m + HAUTEUR_IMAGE_JOUEUR + HAUTEUR_IMAGE_CARTE_DOS / 2, None, 60, NOIR, centrer=True)
        SCREEN.blit(IMAGE_COURONNE, (int(self.xSecondaire + m + LARGEUR_IMAGE_CARTE_DOS + (HAUTEUR_CARRE_JOUEUR_SECONDAIRE - LARGEUR_IMAGE_CARTE_DOS - m - 3) / 2 - LARGEUR_IMAGE_COURONNE / 2), ySecondaire + m + HAUTEUR_IMAGE_JOUEUR - 2))
        affiche_texte(str(self.nbPointsDeVictoire), int(self.xSecondaire + m + LARGEUR_IMAGE_CARTE_DOS + (HAUTEUR_CARRE_JOUEUR_SECONDAIRE - LARGEUR_IMAGE_CARTE_DOS - m) / 2 - 3), int(ySecondaire + m + HAUTEUR_IMAGE_JOUEUR + HAUTEUR_IMAGE_COURONNE / 2 - 2) + 3, None, 35, NOIR, centrer=True)

        SCREEN.blit(IMAGE_PETITE_CARTE_DOS_SPECIALE, (int(self.xSecondaire + m + LARGEUR_IMAGE_CARTE_DOS - 4), ySecondaire + m + HAUTEUR_IMAGE_JOUEUR + HAUTEUR_IMAGE_CARTE_DOS - HAUTEUR_IMAGE_PETITE_CARTE_SPECIALE))
        affiche_texte(str(len(self.listeCartesSpeciales)), int(self.xSecondaire + m + LARGEUR_IMAGE_CARTE_DOS - 4 + LARGEUR_IMAGE_PETITE_CARTE_SPECIALE / 2), int(ySecondaire + m + HAUTEUR_IMAGE_JOUEUR + HAUTEUR_IMAGE_CARTE_DOS - HAUTEUR_IMAGE_PETITE_CARTE_SPECIALE / 2), None, 35, NOIR, centrer=True)
        SCREEN.blit(IMAGE_PETITE_CARTE_CHEVALIER, (int(self.xSecondaire + m + LARGEUR_IMAGE_CARTE_DOS - 4 - 5 + LARGEUR_IMAGE_PETITE_CARTE_SPECIALE), ySecondaire + m + HAUTEUR_IMAGE_JOUEUR + HAUTEUR_IMAGE_CARTE_DOS - HAUTEUR_IMAGE_PETITE_CARTE_SPECIALE))
        affiche_texte(str(self.nbCartesChevaliersRetournees), int(self.xSecondaire + m + LARGEUR_IMAGE_CARTE_DOS - 4 - 5 + LARGEUR_IMAGE_PETITE_CARTE_SPECIALE + LARGEUR_IMAGE_PETITE_CARTE_SPECIALE / 2), int(ySecondaire + m + HAUTEUR_IMAGE_JOUEUR + HAUTEUR_IMAGE_CARTE_DOS - HAUTEUR_IMAGE_PETITE_CARTE_SPECIALE / 2), None, 35, NOIR, centrer=True)
        ...
        if self.points3chevaliers:
            p += 2
        if p > 0:
            pygame.gfxdraw.filled_circle(SCREEN, self.xSecondaire + HAUTEUR_CARRE_JOUEUR_SECONDAIRE, ySecondaire, 22, NOIR)
            pygame.gfxdraw.filled_circle(SCREEN, self.xSecondaire + HAUTEUR_CARRE_JOUEUR_SECONDAIRE, ySecondaire, 20, BLANC)
            affiche_texte('+{}'.format(p), self.xSecondaire + HAUTEUR_CARRE_JOUEUR_SECONDAIRE, ySecondaire, None, 38, NOIR, centrer=True)
    ...
    def affichePrincipal(self):
        ...
        if c == BLANC:
            c = NOIR
        affiche_texte(f'Vous : {self.pseudo.upper()}', int(x + 2 * m + LARGEUR_IMAGE_JOUEUR), int(y + m + HAUTEUR_IMAGE_JOUEUR / 2 - 34 / 4), None, 34, c)
...
# ==========================================================
class Route():
    def __init__(self, id, joueur, arete):
        self.id = id
        #self.joueur = joueur
        self.couleur = joueur.couleur
        self.idArete = arete.id
        arete.contenu = self.id
        self.x1 = arete.x1
        self.y1 = arete.y1
        self.x2 = arete.x2
        self.y2 = arete.y2
...
# ==========================================================
class Colonie():
    def __init__(self, id, joueur, sommet):
        self.id = id
        #self.joueur = joueur
        self.couleur = joueur.couleur
        joueur.nbPointsDeVictoire += 1
